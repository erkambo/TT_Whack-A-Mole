...
async def play_to_score(dut, target_score):
    ...
    while dut.led_score.value.integer < target_score:
        # Wait for a pattern
        await RisingEdge(dut.clk)
        pattern = dut.dut.pattern.value.integer
        if pattern == 0:
...
async def wait_for_score_increment(dut, old_score, timeout=1000):
    for _ in range(timeout):
        await RisingEdge(dut.clk)
        new_score = dut.led_score.value.integer
...
async def test_game_end_display(dut):
    ...
    for _ in range(2):
        ...
        print(f"[test_game_end_display] FSM Latched Pattern: {pattern:07b}, Score before: {dut.led_score.value.integer}")
        await press_pattern(dut, pattern, hold_cycles=5)
        new_score = await wait_for_score_increment(dut, dut.led_score.value.integer - 1)
...
async def test_multi_segment_score(dut):
    ...
    for _ in range(1000):
        ...
        if bin(pattern).count('1') >= 2:
            break
    await wait_some_cycles(dut, 3)
    print(f"[test_multi_segment_score] FSM Latched Pattern: {pattern:07b}, Score before: {dut.led_score.value.integer}")
    await press_pattern(dut, pattern, hold_cycles=5)
    new_score = await wait_for_score_increment(dut, dut.led_score.value.integer - 1)
